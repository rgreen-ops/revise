...
## Done this session
- ✅ **Downloads now shows REAL brochures** — rebuilt as a dynamic page (`[ricoman_downloads]` / `ricoman_downloads_page_html`): brochure cards auto-wire to the matching PDF in the media library (`ricoman_find_brochure_pdf`) and become real downloads; if a brochure isn't uploaded yet the card says "Request" → /contact/. All-LDT / All-Revit buttons show live file counts. Rendered via an override in acf-pages so it always shows (no dependence on a migration/old block content). New Downloads CSS.
- ✅ **Fix old links at source** — broadened the auto-rewriter to nav menus, excerpts, widgets and nav/html/button blocks (not just post content), AND added a one-click **Links & Redirects → "Rewrite old links in content"** button that rewrites old `/product/…` + category-prefixed URLs embedded in all pages/posts/projects/news and saves them (no redirect hop).
