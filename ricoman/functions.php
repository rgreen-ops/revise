...
require_once get_theme_file_path( 'inc/wizard-images.php' ); // Ricoman → Wizard Images: pick the wizard card pictures.
...
require_once get_theme_file_path( 'inc/text-cleanup.php' );    // Ricoman → Clean Import Text: repair CSV mojibake in product + variant data.
require_once get_theme_file_path( 'inc/product-editor.php' );  // Custom Product Page Editor (edit + live preview).
require_once get_theme_file_path( 'inc/product-downloads.php' );// Reliable per-product Downloads manager (files save directly, bypassing the editor).
...
require_once get_theme_file_path( 'inc/collections.php' );    // Product Collections (range grouping): taxonomy + image/desc/landing fields + counts.
...
require_once get_theme_file_path( 'inc/cta.php' );           // Canonical lighting-design CTA banner (one design everywhere)
...
require_once get_theme_file_path( 'inc/go-live.php' );        // Go Live: launch pre-flight checklist + one-click finaliser.
require_once get_theme_file_path( 'inc/configurator-visual.php' ); // Visual (tap-through) variant configurator — per-product alternative to the table.
