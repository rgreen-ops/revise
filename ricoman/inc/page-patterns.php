...
add_action( 'init', function () {
	...
	$p['home-cta'] = array( 'Home · CTA', $cover( $u( 'office1.webp' ),
		...
		$buttons( $btn( 'Start a Project', '/lighting-design/' ) . $btn( 'Talk to the team', '/about/' ), true ), 60, 'center center', 70 ) );
	...
	$p['mfg-cta'] = array( 'Manufacturing · CTA', $cover( $u( 'workshop.webp' ), '<!-- wp:heading {"textAlign":"center","level":2} --><h2 class="wp-block-heading has-text-align-center">Want to see it for yourself?</h2><!-- /wp:heading --><!-- wp:paragraph {"align":"center"} --><p class="has-text-align-center">Book a visit to the Manchester facility, or send us a project and let our team spec it end to end.</p><!-- /wp:paragraph -->' . $buttons( $btn( 'Book a visit', '/about/' ) . $btn( 'Start a project →', '/lighting-design/' ), true ), 52, 'center center', 70 ) );
	 ...
	 * click-to-edit: change the text, and swap the middle photo (select the image
	 * inside the photo card and use Replace). Built from Group + Image (no Cover
	 * block) so it never trips Gutenberg's cover-block validation. */
	...
	$oncard_img = function ( $url, $h, $body ) {
		// Photo sits BEHIND the heading + text (positioned by .rm-oncard--img in
		// the CSS). Plain Group + Image blocks, so no Cover-block validation issues.
		return '<!-- wp:column --><div class="wp-block-column"><!-- wp:group {"className":"rm-oncard rm-oncard--img","layout":{"type":"constrained"}} --><div class="wp-block-group rm-oncard rm-oncard--img">'
			. '<!-- wp:image {"sizeSlug":"large","className":"rm-oncard-photo"} --><figure class="wp-block-image size-large rm-oncard-photo"><img src="' . $url . '" alt=""/></figure><!-- /wp:image -->'
			. '<!-- wp:heading {"level":3} --><h3 class="wp-block-heading">' . $h . '</h3><!-- /wp:heading -->'
			. $body
			. '</div><!-- /wp:group --></div><!-- /wp:column -->';
	};
	$p['cards-image-text'] = array( 'Cards · Image + text (3-up)', $sec(
		...
		. $oncard_img( $u( 'rico-making.webp' ), 'On spec.',
			'<!-- wp:paragraph --><p>Our product development team creates products that boast excellent performance and class-leading technology. We can also make swift and efficient customisations in order to <strong>create products that are truly specification-compliant.</strong></p><!-- /wp:paragraph -->' )
		. $oncard_text( 'On time.',
	...
	$feature = function ( $a ) use ( $hdr_split, $sec, $shead, $eyebrow, $para, $aud, $twocol, $image, $checklist, $buttons, $btn, $cover, $faqblock, $darkgroup ) {
		...
		$out .= $cover( $a['cta_img'],
			...
			. $buttons( $btn( $a['cta1'][0], $a['cta1'][1] ) . $btn( 'Talk to the team →', '/contact/' ), true ),
			52, 'center center', 70 );
		return $out;
	};
	...
	// Mirrors the proven $ptile cover-in-column structure (px min-height, position
	// class, no cover-level text colour — inner blocks carry their own) so it
	// validates cleanly. No HTML entities inside blocks (they trip block
	...
	$p['home-shoutout'] = array( 'Home · Product shoutout',
		'<!-- wp:columns {"align":"full","className":"rm-shout"} --><div class="wp-block-columns alignfull rm-shout">'
		. $shout_panel( $u( 'ceiling.webp' ), 'Featured', 'Zodiac 48V Magnetic Track', 'One modular rail with endless configurations. Spots, linear and pendants for retail, gallery and hospitality.', 'Explore the range', '/product-category/48v-track/' )
		. $shout_panel( $u( 'warm-int.webp' ), 'Featured', 'Estrella Pro Linear', 'Architectural linear light for offices and workspaces. Recessed, surface or suspended.', 'Explore the range', '/product-category/led-linear-lighting/' )
		. '</div><!-- /wp:columns -->'
	);
