...
add_action( 'init', function () {
	...
	$image   = function ( $url, $href = '', $alt = '' ) { $i = '<img src="' . $url . '" alt="' . esc_attr( $alt ) . '" loading="lazy" decoding="async"/>'; if ( $href ) { return '<!-- wp:image {"linkDestination":"custom"} --><figure class="wp-block-image size-large"><a href="' . $href . '">' . $i . '</a></figure><!-- /wp:image -->'; } return '<!-- wp:image {"sizeSlug":"large"} --><figure class="wp-block-image size-large">' . $i . '</figure><!-- /wp:image -->'; };
	...
	$p['about-contact'] = array( 'About · Contact (dark)', $darkgroup( $twocol( $eyebrow( 'Get in touch' ) . $shead( 'Talk to the team' ) . $para( 'Metroplex Business Park, 520 Broadway, M50 2UE, Manchester, UK' ) . $para( '<strong>0161 451 5913</strong><br>sales@ricoman.com' ) . $para( 'Mon–Thu 8:30–17:00 · Fri 8:30–16:00' ), $image( $u( 'rico-office.jpg' ), '', 'Ricoman commercial office lighting scheme, Manchester' ) ) ) );
	...
	/* ===== Downloads ===== */
	$dlcard = function ( $title, $desc, $href ) { return '<!-- wp:column --><div class="wp-block-column"><!-- wp:group {"className":"rm-card rm-soft","style":{"spacing":{"padding":{"top":"var:preset|spacing|40","bottom":"var:preset|spacing|40","left":"var:preset|spacing|40","right":"var:preset|spacing|40"}}},"layout":{"type":"constrained"}} --><div class="wp-block-group rm-card rm-soft" style="padding:var(--wp--preset--spacing--40)"><!-- wp:heading {"level":3} --><h3 class="wp-block-heading"><a href="' . $href . '">' . $title . ' &darr;</a></h3><!-- /wp:heading --><!-- wp:paragraph {"textColor":"muted","fontSize":"small"} --><p class="has-muted-color has-text-color has-small-font-size">' . $desc . '</p><!-- /wp:paragraph --></div><!-- /wp:group --></div><!-- /wp:column -->'; };
	...
	$p['custom-intro'] = array( 'Customisation · Intro', $sec( $twocol( $eyebrow( 'Made to your spec' ) . $shead( 'Bespoke as standard' ) . $para( 'Tell us the geometry, the look and the performance you need. Our in-house design and manufacturing teams turn it into a buildable, costed luminaire — made to order on an average six-day UK lead.', true ) . $buttons( $btn( 'Design a Flow+ run →', '/flow-designer/' ) . $btn( 'Talk to our designers', '/contact/', false ) ), $image( $u( 'rico-betfred-flow.jpg' ), '', 'Bespoke Flow+ curved linear lighting at Betfred HQ' ) ) ) );
	$p['custom-cta'] = array( 'Customisation · CTA', $cover( $u( 'rico-soundslikelight.png' ), '<!-- wp:heading {"textAlign":"center","level":2} --><h2 class="wp-block-heading has-text-align-center">Start a bespoke project</h2><!-- /wp:heading --><!-- wp:paragraph {"align":"center"} --><p class="has-text-align-center">Use the Flow+ Designer to draw your run, or send us a sketch and we&rsquo;ll spec it end to end.</p><!-- /wp:paragraph -->' . $buttons( $btn( 'Open Flow+ Designer', '/flow-designer/' ) . $btn( 'Talk to the team →', '/contact/', false ), true ), 52, 'center center', 70 ) );

	/* ===== Contact ===== */
	$p['contact-hero'] = array( 'Contact · Hero', $cover( $u( 'rico-office.jpg' ), $eyebrow( 'Contact' ) . '<!-- wp:heading {"level":1,"style":{"typography":{"fontWeight":"500","fontSize":"clamp(2.6rem, 6vw, 5rem)","lineHeight":"1"}}} --><h1 class="wp-block-heading" style="font-size:clamp(2.6rem, 6vw, 5rem);font-weight:500;line-height:1">Let&rsquo;s talk lighting.</h1><!-- /wp:heading -->' . $para( 'Send us a project, a drawing or a question — our Manchester team will get straight back to you.' ), 52, 'bottom left', 50 ) );
	$p['contact-body'] = array( 'Contact · Form &amp; details', $sec( '<!-- wp:columns --><div class="wp-block-columns"><!-- wp:column {"width":"58%"} --><div class="wp-block-column" style="flex-basis:58%">' . $eyebrow( 'Send an enquiry' ) . $shead( 'Tell us about your project' ) . '<!-- wp:shortcode -->[ricoman_lead_form]<!-- /wp:shortcode --></div><!-- /wp:column --><!-- wp:column {"width":"42%"} --><div class="wp-block-column" style="flex-basis:42%"><!-- wp:group {"className":"rm-soft","style":{"spacing":{"padding":{"top":"var:preset|spacing|40","bottom":"var:preset|spacing|40","left":"var:preset|spacing|40","right":"var:preset|spacing|40"}}},"layout":{"type":"constrained"}} --><div class="wp-block-group rm-soft" style="padding:var(--wp--preset--spacing--40)"><!-- wp:heading {"level":3} --><h3 class="wp-block-heading">Get in touch</h3><!-- /wp:heading -->' . $para( 'Metroplex Business Park<br>520 Broadway, M50 2UE<br>Manchester, UK' ) . $para( '<strong>0161 451 5913</strong><br>sales@ricoman.com' ) . $para( 'Mon–Thu 8:30–17:00 · Fri 8:30–16:00' ) . '</div><!-- /wp:group --></div><!-- /wp:column --></div><!-- /wp:columns -->' ) );
