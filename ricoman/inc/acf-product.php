...
function ricoman_pf_gallery_js() {
	return '<script>(function(){function ready(fn){if(document.readyState!=="loading"){fn();}else{document.addEventListener("DOMContentLoaded",fn);}}ready(function(){var w=document.querySelector(".rm-cfghero-wrap")||document.querySelector(".rm-pdp-gallery");if(!w)return;var im=w.querySelector(".rm-cfg-img");function applyFit(img){if(!img||!img.naturalWidth)return;try{var c=document.createElement("canvas"),s=24;c.width=s;c.height=s;var x=c.getContext("2d");x.drawImage(img,0,0,s,s);var d=x.getImageData(0,0,s,s).data,t=false;for(var i=3;i<d.length;i+=4){if(d[i]<240){t=true;break;}}img.classList.toggle("rm-cutout",t);}catch(e){img.classList.remove("rm-cutout");}}if(im){im.addEventListener("load",function(){applyFit(im);});if(im.complete)applyFit(im);}function bind(sel){w.querySelectorAll(sel).forEach(function(b){b.addEventListener("click",function(){if(b.dataset.img&&im){im.src=b.dataset.img;}var p=b.parentNode;p.querySelectorAll(sel).forEach(function(x){x.classList.remove("on");});b.classList.add("on");});});}bind(".rm-cv-sw");bind(".rm-cfg-thumb");w.querySelectorAll(".rm-gtab").forEach(function(t){t.addEventListener("click",function(){if(t.disabled)return;w.querySelectorAll(".rm-gtab").forEach(function(x){x.classList.remove("on");});t.classList.add("on");var tab=t.dataset.tab;w.querySelectorAll(".rm-gthumbs .rm-cfg-thumb").forEach(function(th){th.style.display=(tab==="all"||th.dataset.tab===tab)?"":"none";});});});var lb=w.querySelector(".rm-lightbox"),lbi=lb?lb.querySelector(".rm-lightbox-img"):null;if(lb&&lbi&&im){function open(){lbi.src=im.src;lb.hidden=false;document.body.style.overflow="hidden";}im.style.cursor="zoom-in";im.addEventListener("click",open);var hint=w.querySelector(".rm-zoom-hint");if(hint){hint.style.cursor="zoom-in";hint.addEventListener("click",open);}function cl(){lb.hidden=true;document.body.style.overflow="";}lb.addEventListener("click",function(e){if(e.target===lb||e.target.classList.contains("rm-lightbox-x"))cl();});document.addEventListener("keydown",function(e){if(e.key==="Escape")cl();});}});})();</script>';
}
