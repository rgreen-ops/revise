...
function ricoman_pf_metrics( $pid ) {
	...
	if ( is_array( $pre ) && isset( $pre['_v'] ) && (int) $pre['_v'] >= 4 ) {
		return $memo[ $pid ] = $pre;
	}
...
add_shortcode( 'ricoman_cat_filter', function ( $atts ) {
	...
	// Cut-out slider — only on luminaire categories (those that also have lumens or
	// wattage). Accessory categories can carry a stray cut-out value on one odd
	// product; a lone cut-out slider there just hides everything else, so gate it out.
...
function ricoman_pcard_html( $url, $pid, $img, $sub, $mx, $co, $fslug, $cats, $mts, $fins, $isnew = false, $is_accessory = false, $is_default = false, $img2 = '' ) {
