...
$pcard = function ( $url, $label, $title, $spec, $href ) {
	$img = '<!-- wp:image {"linkDestination":"custom"} --><figure class="wp-block-image size-large"><a href="' . $href . '"><img src="' . $url . '" alt="' . esc_attr( $title ) . '" loading="lazy" decoding="async"/></a></figure><!-- /wp:image -->';
