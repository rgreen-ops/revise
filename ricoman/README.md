...
| **Content Transporter** | `inc/transporter.php` | **Tools → Content Transporter.** Migrate existing pages into the theme one at a time: paste URL(s), pick the post type + template, and it fetches the page, **sideloads every image** into the Media Library, maps title/description/hero/body into the right fields and **blocks** (headings, paragraphs, lists, images, quotes), and drops anything it can't confidently place into an **"Unsorted imported content"** box on the draft for manual placement. |
