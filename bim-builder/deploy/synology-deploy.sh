#!/bin/sh
# Bim Builder — Synology auto-deploy (pull model).
#
# Fetches the latest committed app from GitHub and syncs the /bim-builder folder
# into your Web Station web root. Run it from DSM > Control Panel > Task Scheduler
# on a schedule (e.g. every 5 minutes) so the site updates itself whenever a new
# commit lands on `main` — no manual zip dropping.
#
...
BRANCH="main"
TARGET="/volume1/web/bim-builder"          # Web Station web root for the app
...
if [ ! -d "${SRC}bim-builder" ]; then
  log "ERROR: bim-builder/ not found in the downloaded archive — aborting."
  exit 1
fi

mkdir -p "$TARGET"
rsync -a --delete --exclude '.revise_token' "${SRC}bim-builder/" "$TARGET/"
